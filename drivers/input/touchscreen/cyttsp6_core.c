...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0)) /* For Kernel version < 4.19 */
...
#else /* For Kernel version >= 4.19 */
static void cyttsp6_watchdog_timer(struct timer_list *handle)
{
    struct cyttsp6_core_data *cd = from_timer(cd, handle,watchdog_timer);
    dev_vdbg(cd->dev, "%s: Timer triggered\n", __func__);
    if (!work_pending(&cd->watchdog_work))
        schedule_work(&cd->watchdog_work);
}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	timer_setup(&cd->watchdog_timer, cyttsp6_watchdog_timer, TIMER_INIT_FLAGS);
